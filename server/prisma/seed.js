...
async function main() {
    ...
    await prisma.user.create({
        data: {
            email: 'admin@school.com', password, role: client_1.UserRole.SUPER_ADMIN,
    ...
    for (let i = 0; i < 5; i++) {
        const tUser = await prisma.user.create({
            data: {
                email: `teacher${i + 1}@school.com`, password, role: client_1.UserRole.TEACHER,
                teacherProfile: {
        ...
        if (tUser.teacherProfile)
            teachers.push(tUser.teacherProfile);
    }
    ...
    for (let i = 0; i < 50; i++) {
        const sUser = await prisma.user.create({
            data: {
                email: `student${i + 1}@school.com`, password, role: client_1.UserRole.STUDENT,
                studentProfile: {
                    create: {
        ...
        const studentFee = await prisma.studentFee.create({
            data: { studentId, feeStructureId: feeStruct.id, status: client_1.FeeStatus.PARTIAL }
        });
